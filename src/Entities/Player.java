			...
			for(int j = 0; j < b[0].length; j++) {
				...
				if(b[i][j].getID() != 0) {
					...
					if(Collision.playerBlcok(new Point(iX + (int)GameState.xOffset + 1, iY + (int)GameState.yOffset), b[i][j]) 
							|| Collision.playerBlcok(new Point(iX + width + (int)GameState.xOffset - 2, iY + (int)GameState.yOffset), b[i][j])) {
						jumping = false;
						falling = true;
					}
					...
					if(Collision.playerBlcok(new Point(iX + (int)GameState.xOffset + 2, iY + height + (int)GameState.yOffset + 1), b[i][j])
							|| Collision.playerBlcok(new Point(iX + width + (int)GameState.xOffset - 2, iY + height + (int)GameState.yOffset + 1), b[i][j])) {
